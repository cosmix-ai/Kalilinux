...
local v = "1.6" -- Updated version with fly function and bug fixes
...
local guiSettings = {
    ...
    fontSize = 14,
    guiSize = UDim2.new(0, 450, 0, 400), -- Adjusted size to fit screen better
    minimized = false,
...
local function createSettingsMenu(content, buttons, mainFrame)
    for _, c in ipairs(content:GetChildren()) do
        if c:IsA("Frame") or c:IsA("TextBox") or c:IsA("TextButton") or c:IsA("TextLabel") or c:IsA("ScrollingFrame") then
            c:Destroy()
        end
    end
            ...
            if humanoid then
                ...
                flyConnection = RunService.Heartbeat:Connect(function()
                    ...
                    if rootPart and bodyVelocity and bodyGyro then
                        -- Get control inputs
                        local forward = UserInput:IsKeyDown(Enum.Key.W) and 1 or UserInput:IsKeyDown(Enum.Key.S) and -1 or 0
                        local right = UserInput:IsKeyDown(Enum.Key.D) and 1 or UserInput:IsKeyDown(Enum.Key.A) and -1 or 0
                        local up = UserInput:IsKeyDown(Enum.Key.Space) and 1 or UserInput:IsKeyDown(Enum.Key.LeftControl) and -1 or 0
                        
                        -- Calculate movement direction
                        local camera = workspace.CurrentCamera
                        local cf = camera.CFrame
                        local direction = (cf.LookVector * forward) + (cf.RightVector * right) + (Vector3.new(0, 1, 0) * up)
...
function loadMainGUI()
    ...
    featureScroll.Size = UDim2.new(0, 150, 0, 330) -- Adjusted height
    ...
    local features = {
        ...
        {text="บิน", name="fly"}, -- Added fly feature
        {text="ตั้งค่า", name="settings"},
    }

    local buttons = {}
    for i, feat in ipairs(features) do
        local b = createStyledButton(featureScroll, feat.text, UDim2.new(1, 0, 0, 40), UDim2.new(0, 0, 0, (i-1)*50)) -- Smaller buttons
        b.Name = feat.name
        b.LayoutOrder = i
        buttons[feat.name] = b
    ...
    buttons.speed.MouseButton1Click:Connect(function()
        ...
        local setBtn = createStyledButton(content, "ตั้งค่า", UDim2.new(1, -20, 0, 40), UDim2.new(0, 10, 0, 60))
    ...
    buttons.fly.MouseButton1Click:Connect(function()
        toggleFly()
        buttons.fly.Text = flyEnabled and "บิน (เปิด)" or "บิน (ปิด)"
        buttons.fly.BackgroundColor3 = flyEnabled and Color3.fromRGB(0, 170, 0) or guiSettings.accentColor
    end)
