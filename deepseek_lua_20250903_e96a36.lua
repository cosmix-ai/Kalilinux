...
local v = "1.6" -- Updated version with fixes
...
local function enableFlight()
    ...
    -- Enable flight
    flyObjects.Gyro.MaxTorque = Vector3.new(10000, 10000, 10000)
    flyObjects.Velocity.MaxForce = Vector3.new(10000, 10000, 10000)
    ...
    flightConnection = RunService.Heartbeat:Connect(function()
        if not flying or not flyEnabled or not player.Character or not flyObjects.Gyro or not flyObjects.Velocity then
            flightConnection:Disconnect()
            return
        end
        
        local newRoot = player.Character:FindFirstChild("HumanoidRootPart")
        if not newRoot or flyObjects.Gyro.Parent ~= newRoot or flyObjects.Velocity.Parent ~= newRoot then
            if flyEnabled then
                -- Reattach flight objects to new root part
        ...
        flyObjects.Gyro.CFrame = workspace.CurrentCamera.CoordinateFrame
        ...
        flyObjects.Velocity.Velocity = moveDirection
    end)
end
...
local function toggleFlight()
    ...
    if flyEnabled then
        ...
        -- Re-enable flight if character respawns
